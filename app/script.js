const appURL = () => {
    ...
    return protocol + location.hostname + (location.hostname == 'localhost' ? ':5000' : '');
};
                    ...
                    peer.createAnswer(
                        function (local_description) {
                            peer.setLocalDescription(
                                ...
                                function (err) {
                                    console.log(err);
                                    alert('Câu trả lời setLocalDescription failed!');
                                }
                            );
                        },
                        function (error) {
                            console.log('Lỗi khi kết nối LocalDescription: ', error);
                        }
                    );
                }
            },
...
function setup_local_media(callback, errorback) {
    ...
    navigator.mediaDevices
        ...
        .catch(() => {
            alert('Ứng dụng này sẽ không hoạt động nếu không có quyền truy cập máy ảnh / micrô.');
...
const swapCamera = () => {
    ...
    navigator.mediaDevices
        ...
        .catch(err => {
            console.log(err);
            alert('Lỗi khi chuyển đổi máy ảnh');
        });
};